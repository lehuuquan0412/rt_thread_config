...
static struct stm32_can_config can_config[] =
{
#ifdef BSP_USING_CAN1
    CAN1_CONFIG,
#endif
...
static rt_err_t stm32_can_clk_enable(CAN_TypeDef *Instance)
{
...
static rt_err_t stm32_gpio_configure(struct stm32_can_config *config)
{
    //int can_num = 0;
...
//static struct stm32_can can_obj[sizeof(can_config) / sizeof(can_config[0])] = {0};
...
static rt_err_t _can_config(struct rt_can_device *can, struct can_configure *cfg)
{
    ...
    drv_can = rt_container_of(can, struct stm32_can, device);
    RT_ASSERT(drv_can);

    stm32_can_clk_enable(drv_can->CanHandle.Instance);
    //stm32_gpio_configure(drv_can->config);
#ifdef BSP_USING_CAN1
    stm32_gpio_configure(&can_config[CAN1_INDEX]);
#endif

#ifdef BSP_USING_CAN2
    stm32_gpio_configure(&can_config[CAN2_INDEX]);
#endif
...
//            can->hdr = (struct rt_can_hdr*)rt_malloc(sizeof(struct rt_can_hdr)*(filter_cfg->count));
//            for (int i = 0; i < filter_cfg->count; i++)
//            {
//                can->hdr[i].filter = filter_cfg->items[i];
//            }
            /* get default filter */
            for (int i = 0; i < filter_cfg->count; i++)
            {
                ...
                // drv_can->FilterConfig.FilterScale
                ...
                drv_can->FilterConfig.FilterActivation = filter_cfg->actived;
                /* Filter conf */
                HAL_CAN_ConfigFilter(&drv_can->CanHandle, &drv_can->FilterConfig);
            }
        }
...
static rt_size_t _can_sendmsg(struct rt_can_device *can, const void *buf, rt_uint32_t boxno)
{
            ...
            (state == HAL_CAN_STATE_LISTENING))
    {
        ...
        if (RT_CAN_STDID == pmsg->ide)
        {
            hcan->Instance->sTxMailBox[boxno].TIR |= (txheader.StdId << CAN_TI0R_STID_Pos) | txheader.RTR;
        }
        else
        {
...
static rt_size_t _can_recvmsg(struct rt_can_device *can, void *buf, rt_uint32_t fifo)
{
